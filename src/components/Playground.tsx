...
import { Play, RotateCcw, Copy, Download, Sparkles, Check, CheckCircle2, AlertTriangle } from "lucide-react";
...
export default function Playground() {
  ...
  const handleRun = () => {
    ...
    setConsoleLogs(["[System] Transpiling TypeScript...", "[System] Executing compiled JavaScript..."]);
  ...
  return (
    <div className="flex flex-col lg:flex-row h-[calc(100vh-130px)] md:h-[calc(100vh-84px)] bg-surface text-on-surface" id="playground-page">
      {/* Editor Pane */}
      <div className="flex-1 flex flex-col border-r border-outline-variant/30" id="playground-editor-pane">
        {/* Editor Toolbar */}
        <div className="flex items-center justify-between px-6 py-3 bg-surface-container-high/60 border-b border-outline-variant/30">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-[10px] font-mono font-bold uppercase tracking-wider text-on-surface-variant">Sandbox Code</span>
          </div>
        ...
        <div className="flex-1 w-full h-full relative" id="monaco-editor-container">
          <Editor
            height="100%"
            language="typescript"
            theme="vs-dark"
      ...
      <div className="w-full lg:w-96 flex flex-col bg-surface-container-low border-l border-outline-variant/30" id="playground-output-pane">
        ...
        <div className="p-4 border-b border-outline-variant/30 bg-surface-container-high/20">
          ...
          {errors.length === 0 ? (
            <div className="flex items-center gap-2.5 p-3.5 rounded border border-emerald-500/20 bg-emerald-500/5 text-emerald-400 text-sm">
              ...
              <div>
                <p className="font-semibold uppercase tracking-wider text-xs">Compilation clean</p>
                <p className="text-xs text-emerald-500/80 mt-0.5 font-sans">Type safety rules are 100% satisfied.</p>
              </div>
            </div>
          ) : (
            <div className="flex flex-col gap-2">
              <div className="flex items-center gap-2.5 p-3.5 rounded border border-rose-500/25 bg-rose-500/5 text-rose-400 text-sm mb-1">
                ...
                <div>
                  <p className="font-semibold uppercase tracking-wider text-xs">Found {errors.length} compiler issues</p>
                  <p className="text-xs text-rose-500/80 mt-0.5 font-sans">Correct the errors highlighted below.</p>
                </div>
              </div>
              <div className="max-h-40 overflow-y-auto flex flex-col gap-1.5 pr-1">
                {errors.map((err, idx) => (
                  <div key={idx} className="p-2.5 bg-slate-900 border border-slate-800 rounded text-xs flex flex-col gap-1">
                    ...
                    <p className="text-slate-300 font-sans leading-relaxed">{err.message}</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="flex-1 flex flex-col p-4">
          ...
          <div className="flex-1 bg-slate-900/30 border border-slate-800 rounded p-4 font-mono text-xs overflow-y-auto flex flex-col gap-2 text-slate-300 min-h-[200px]">
            {consoleLogs.length === 0 ? (
              <span className="text-slate-600 italic font-sans">No output yet. Click "Run Code" to compile and execute.</span>
            ) : (
              consoleLogs.map((log, idx) => (
                <div key={idx} className={`whitespace-pre-wrap leading-relaxed ${
                  log.startsWith("❌") ? "text-rose-400 font-medium" : 
                  log.startsWith("[System]") ? "text-slate-500" : "text-sky-300"
                }`}>
                  {log}
                </div>
              ))
