...
import { 
  Trophy, BookOpen, Star, Sparkles, Award, Lock, ChevronRight,
  ShieldAlert, Settings, ShoppingCart, Gamepad2, Info, Compass
} from "lucide-react";
...
export default function Home({
  ...
  useEffect(() => {
    ...
    const activeLevel = LEVELS.find(
      (lvl) => unlockedLevelIds.includes(lvl.id) && !completedLevelIds.includes(lvl.id)
    );
  ...
  const handleStartLearning = () => {
    ...
    if (!targetLevelId || !unlockedLevelIds.includes(targetLevelId)) {
      const incompleteUnlocked = LEVELS.find(l => unlockedLevelIds.includes(l.id) && !completedLevelIds.includes(l.id));
  ...
  return (
    <div className="flex-1 bg-surface flex flex-col min-h-screen text-on-surface select-none pb-12 pt-8 px-4" id="home-view">
      ...
      <section className="text-center mb-16 relative max-w-[1280px] mx-auto px-6 md:px-12" id="hero-section">
        ...
        <div className="mb-6 inline-flex p-4 rounded-full bg-surface-container-high border border-outline-variant shadow-lg active-glow">
          <span className="material-icons-out text-primary text-4xl" style={{ fontVariationSettings: "'FILL' 1" }}>psychology</span>
        </div>

        <h1 className="font-sans text-4xl md:text-6xl font-extrabold mb-6 tracking-tight">
          Master TypeScript through <span className="vibrant-gradient">Play</span>
        </h1>
        
        <p className="max-w-2xl mx-auto text-on-surface-variant text-base md:text-lg mb-10 leading-relaxed">
          Learn TypeScript types and concepts through an interactive game. Progress through levels, solve challenges, and become a TypeScript expert.
        </p>
        ...
        <div className="mt-12 flex justify-center gap-4 flex-wrap">
          ...
          >
            <span className="material-icons-out text-sm">settings</span> Difficulty
          </button>
          <button 
            onClick={() => setShowSubModal({
          ...
          >
            <span className="material-icons-out text-sm">shopping_cart</span> Shop
          </button>
          <button 
            onClick={() => setShowSubModal({
          ...
          >
            <span className="material-icons-out text-sm">sports_esports</span> Mini Games
          </button>
        </div>
      </section>
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-24 max-w-[1100px] w-full mx-auto px-6" id="stats-grid">
        <div className="glass-card p-6 rounded-xl text-center group hover:border-primary/50 transition-colors">
          <p className="font-mono text-xs uppercase tracking-wider text-on-surface-variant mb-2 group-hover:text-primary">Points</p>
          <h3 className="text-3xl md:text-4xl font-extrabold text-on-surface font-sans">{xp} XP</h3>
        </div>
        <div className="glass-card p-6 rounded-xl text-center group hover:border-primary/50 transition-colors">
          <p className="font-mono text-xs uppercase tracking-wider text-on-surface-variant mb-2 group-hover:text-primary">Completed</p>
          <h3 className="text-3xl md:text-4xl font-extrabold text-on-surface font-sans">{completedLevelIds.length} / {LEVELS.length}</h3>
        </div>
        <div className="glass-card p-6 rounded-xl text-center group hover:border-primary/50 transition-colors">
          <p className="font-mono text-xs uppercase tracking-wider text-on-surface-variant mb-2 group-hover:text-primary">Wizard Level</p>
          <h3 className="text-3xl md:text-4xl font-extrabold text-on-surface font-sans">Lvl {calculatedLevel}</h3>
        </div>
        <div className="glass-card p-6 rounded-xl text-center group hover:border-primary/50 transition-colors">
          <p className="font-mono text-xs uppercase tracking-wider text-on-surface-variant mb-2 group-hover:text-primary">Rank Status</p>
            ...
            {STAGES.map((stage, sIdx) => {
              ...
              return (
                <div key={stage.id} className="relative z-10 w-full" id={`stage-node-${stage.id}`}>
                  ...
                  <div className={`absolute left-1/2 -translate-x-1/2 -top-3 w-8 h-8 rounded-full border-2 bg-surface-container flex items-center justify-center shadow-lg hidden md:flex transition-all duration-300 ${
                    isFullyCompleted ? "border-emerald-500 bg-emerald-950/40" : isStageUnlocked ? "border-primary active-glow bg-primary/10" : "border-outline-variant"
                  }`}>
                    {isFullyCompleted ? (
                    ...
                    {isExpanded ? (
                      <div className="glass-card p-6 md:p-8 rounded-2xl border-l-4 border-l-primary glow-primary transition-all duration-300">
                        ...
                        <div className="mt-6 flex justify-end">
                          ...
                          >
                            Enter Active Code Editor <span className="material-icons-out text-sm">arrow_forward_ios</span>
                          </button>
                        </div>
                      </div>
                    ) : (
                      ...
                      >
                        <div className="flex items-center gap-4">
                          <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
                            ...
                            <span className="material-icons-out">
                              {sIdx === 0 ? "rocket_launch" : sIdx === 1 ? "commit" : sIdx === 2 ? "schema" : "extension"}
                            </span>
                          </div>
                          <div>
                        ...
                        <div className="flex items-center gap-2">
                          {!isStageUnlocked ? (
                            ...
                            <span className="material-icons-out text-sm text-on-surface-variant group-hover:text-primary">unfold_more</span>
                          )}
                        </div>
                      </div>
                    )}
      ...
      {showSubModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/90 backdrop-blur-md" id="info-overlay">
          <div className="p-6 bg-surface-container border border-outline-variant rounded-xl max-w-sm w-full flex flex-col gap-4 relative shadow-2xl">
            ...
            <div className="flex items-center gap-3">
              <span className="material-icons-out text-primary text-2xl">info</span>
